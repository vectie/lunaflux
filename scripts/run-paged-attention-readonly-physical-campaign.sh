...
(( registers <= 128 && shared <= 512 && stack == 0 && spill_store == 0 && spill_load == 0 )) || fail 'resource bound exceeded'
...
for exact in 'outcome=paged-attention-readonly-sm120-qualification-pass ' \
  ...
  'dispatch_grid_x_max=128 ' 'dispatch_grid_y=2 ' 'dispatch_block_x=128 ' \
...
printf '%s\n' 'schema=lunaflux-paged-attention-readonly-physical-campaign.v1' \
  ...
  "registers_per_thread=$registers" 'register_bound=128' "static_shared_bytes=$shared" 'static_shared_bound=512' \
