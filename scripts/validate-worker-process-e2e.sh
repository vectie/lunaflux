...
moon build cmd/worker_echo cmd/worker_echo_fail tests/persistent_online_worker \
  tests/worker_process_e2e tests/worker_service_e2e \
...
persistent_worker="$repo_root/_build/native/release/build/tests/persistent_online_worker/persistent_online_worker.exe"

if [ ! -x "$worker" ] || [ ! -x "$failure_worker" ] ||
  [ ! -x "$persistent_worker" ] || [ ! -x "$gate" ] ||
  [ ! -x "$service_gate" ]; then
...
"$gate" "$worker"
"$service_gate" "$worker" "$failure_worker" "$persistent_worker"
