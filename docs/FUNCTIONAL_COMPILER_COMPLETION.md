...
| Operand segmentation | Shared semantic address/segment plan consumed by projection lowerers; preserve useful schedules rather than force losing hoists | Partial: generic concatenated-row plan feeds QKV scalar/matrix binding and lowering plus transfer interval planning; exact caller-controlled expansion budget replaces conservative/count caps. Full QKV transfer-worker schedule integration remains open; previous hoist regressed |
